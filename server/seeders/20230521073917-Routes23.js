...
module.exports = {
  ...
  async up (queryInterface, Sequelize) {
    ...
    const stop = await Stops.findOne({ where:{ name: "Kościuszki Hala Widowiskowo-Sportowa" } });
    const line = await Lines.findOne({ where:{ num_line: "31" } });
    const route = await routeTypes.findOne({ where:{ name: "Powszednie" } });
    const routeOrder={order:'3'}
