...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    for (let line of linesAll) {
      ...
      let routesAllWeek = await Routes.findAll({ where: { id_line: lineObj.id_line, sunday: true } });
        ...
        for (let route of routesAllWeek) {
          ...
          if (before !==null) {
            ...
            if (timeToSeconds('03:00:00') < oldTime && oldTime < timeToSeconds('11:00:00')) {
              timeToAdd = timeToSeconds(datatime.sunday_mor)
              addedTime = (oldTime+timeToAdd) % (24*3600)
              
            } else if (timeToSeconds('11:00:00') < oldTime && oldTime < timeToSeconds('19:00:00')) {
              timeToAdd =timeToSeconds(datatime.sunday_mid)
              addedTime = (oldTime+timeToAdd) % (24*3600)
              
              
            } else {
              timeToAdd = timeToSeconds(datatime.sunday_eve)
              addedTime = (oldTime+timeToAdd) % (24*3600)
            }
