...
exports.addWeek = async (req, res) => {
    ...
    const hsckPass = await Departures.findOne({where:{id_route:route.id_line},order:['num_passage','DESC']})
     let passage   
        ...
        let mapRoute = await Routes.findAll({where:{id_line:data.id_line,},order:['order','ASC']})
        let before = null
       ...
       let first = true
         mapRoute = await Routes.findAll({where:{id_line:data.id_line,},order:['order','DESC']})

         for (let dat of mapRoute){     
            first =  addweekBack(passage,dat,time,before)
        }
